...
export const Channel = () => {
  ...
  const searchTerm = (keyword) => {
    setSearching(true)
    const limit = 30
    const usersRequest = new CometChat.UsersRequestBuilder().setLimit(limit).setSearchKeyword(keyword).build()
  ...
  const addMember = (guid, uid) => {
    let GUID = guid
    let membersList = [new CometChat.GroupMember(uid, CometChat.GROUP_MEMBER_SCOPE.PARTICIPANT)]
        ...
        <div className="callScreen">
          <div className="callScreen__container">
            <div className="call-animation">
              <img className="img-circle" src={channel?.avatar} alt="" width="135" />
            </div>
            {isOutgoingCall ? <h4>Calling {channel?.name}</h4> : <h4>{channel?.name} Calling</h4>}
      ...
      <div className={`channel__details ${!toggle ? 'hide__details' : ''}`}>
        ...
        <div className="channel__detailsBody">
          ...
          <div className="channel__detailsMembers">
            ...
            {members.map((member) => (
              <div key={member?.uid} className={`available__member ${member?.status === 'online' ? 'isOnline' : ''}`}>
                <Avatar src={member?.avatar} alt={member?.name} />
                <Link to={`/users/${member?.uid}`}>{member?.name}</Link>
                <FiberManualRecordIcon />
                {member?.scope !== 'admin' ? (
                  channel?.scope === 'admin' ? (
                    <PersonAddDisabledIcon onClick={() => remMember(id, member?.uid)} title={member?.scope} />
                  ) : (
                    ''
                  )
                ) : (
      ...
      <div className={`channel__details ${!toggleAdd ? 'hide__details' : ''}`}>
        ...
        <div className="channel__detailsBody">
          <form onSubmit={(e) => findUser(e)} className="channel__detailsForm">
            <input placeholder="Search for a user" onChange={(e) => setKeyword(e.target.value)} required />
            <Button onClick={(e) => findUser(e)}>{!searching ? 'Find' : <div id="loading"></div>}</Button>
          </form>
          <hr />
          <div className="channel__detailsMembers">
            <h4>Search Result({users.length})</h4>
            {users.map((user) => (
              <div key={user?.uid} className={`available__member ${user?.status === 'online' ? 'isOnline' : ''}`}>
                <Avatar src={user?.avatar} alt={user?.name} />
                <Link to={`/users/${user?.uid}`}>{user?.name}</Link>
                <FiberManualRecordIcon />
                {currentUser.uid !== user?.uid ? (
                  channel?.scope === 'admin' ? (
                    <PersonAddOutlinedIcon onClick={() => addMember(id, user?.uid)} />
                  ) : (
                    ''
                  )
                ) : (
