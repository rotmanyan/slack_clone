        ...
        <div className="callScreen">
          <div className="callScreen__container">
            <div className="call-animation">
              <img className="img-circle" src={user?.avatar} alt="" width="135" />
            </div>
            {isOutgoingCall ? <h4>Calling {user?.name}</h4> : <h4>{user?.name} Calling</h4>}
            ...
            {users.map((user) => (
              <div key={user?.uid} className={`available__member ${user?.status === 'online' ? 'isOnline' : ''}`}>
                ...
                {currentUser?.uid.toLowerCase() === id.toLowerCase() ? (
                  <PersonAddDisabledIcon onClick={() => remFriend(id, user?.uid)} />
                ) : (
                  ''
                )}
              </div>
